namespace Prompt
{
    ...
    public class PromptRetryPolicy
    {
        ...
        /// <summary>
        /// Async version of Execute that uses Task.Delay instead of Thread.Sleep,
        /// avoiding stack pressure and supporting cancellation.
        /// </summary>
        ...
        /// <summary>
        /// Execute a synchronous operation asynchronously with retry policy.
        /// Uses Task.Delay instead of Thread.Sleep to avoid stack issues with
        /// high retry counts. Delegates to the shared async retry loop.
        /// </summary>
        public Task<RetryResult> ExecuteAsync(
            Func<int, (bool success, string resultOrError)> operation,
            CancellationToken cancellationToken = default)
        {
            return ExecuteAsync(
                (attempt, ct) => Task.FromResult(operation(attempt)),
                cancellationToken);
        }
