...
class FeedAnnotationManager {
    ...
    struct Annotation: Codable, Equatable {
        ...
        let feedName: String
        let selectedText: String       // The quoted/selected passage
        let comment: String            // User's annotation comment
        let category: AnnotationCategory
        let contextBefore: String      // Text before selection (up to 100 chars)
        let contextAfter: String       // Text after selection (up to 100 chars)
        let characterOffset: Int       // Approximate character offset in article
        let createdAt: Date
        var updatedAt: Date
        var isResolved: Bool           // For questions/action items
    ...
    /// Add a new annotation anchored to a text selection.
    @discardableResult
    func addAnnotation(
        articleLink: String,
        articleTitle: String,
    ...
    /// Update the comment and/or category of an existing annotation.
    ...
    /// Toggle resolved state (useful for questions and action items).
    ...
    /// Remove all annotations for a specific article.
    ...
    /// All annotations for a specific article, ordered by character offset.
    ...
    /// All annotations for a specific feed.
    ...
    /// Get unresolved questions and action items.
    ...
    /// Full-text search across selected text, comments, and article titles.
    ...
    /// Annotations created within a date range.
    ...
    /// Most recently annotated articles (unique, ordered by latest annotation).
    ...
    /// Compute aggregate annotation statistics.
    ...
    /// Export all annotations as JSON data.
    ...
    /// Import annotations from JSON data (merges, skipping duplicates by ID).
